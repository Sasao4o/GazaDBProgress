...
Value::Value(TypeId type, const char *data, uint32_t len, bool manage_data) : Value(type) {
  // LOG_DEBUG("IN THE VALUE VAR CHARRRRR CONTEXT __ _ _  _ _ _ _ _ _  _ _  _ ");
  switch (type) {
    case TypeId::VARCHAR:
      if (data == nullptr) {
        value_.varlen_ = nullptr;
...
Value::Value(TypeId type, const std::string &data) : Value(type) {
  switch (type) {
    case TypeId::VARCHAR: {
      manage_data_ = true;
      //LOOKHERE
      // TODO(TAs): How to represent a null string here? (hwa mn wght nzry empty string not null ;)
