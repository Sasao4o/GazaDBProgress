...
// #include "storage/table/tuple_row.h"

// namespace bustub {

// // TODO(Amadou): It does not look like nulls are supported. Add a null bitmap?
// TupleRow::TupleRow(std::vector<Value> values, const Schema *schema) : allocated_(true) {
...
// TupleRow::TupleRow(const TupleRow &other) : allocated_(other.allocated_), rid_(other.rid_), size_(other.size_) {
//   if (allocated_) {
//     delete[] data_;
//   }
//   if (allocated_) {
...
// auto TupleRow::operator=(const TupleRow &other) -> TupleRow & {
//   if (allocated_) {
//     delete[] data_;
//   }
...
// auto TupleRow::GetValue(const Schema *schema, const uint32_t column_idx) const -> Value {
//   assert(schema);
//   assert(data_);
...
// auto TupleRow::KeyFromTuple(const Schema &schema, const Schema &key_schema, const std::vector<uint32_t> &key_attrs)
//     -> Tuple {
...
//   }
//   return TupleRow(values, &key_schema);
// }
...
// auto TupleRow::ToString(const Schema *schema) const -> std::string {
//   std::stringstream os;
